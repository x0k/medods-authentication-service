...
const saveTokeHashQuery = `INSERT INTO refresh_tokens (user_id, device_id, token_hash)
VALUES ($1, $2, $3)
...
func (r *refreshTokensRepository) UpsertTokenHash(
	...
	_, err := r.pool.Exec(ctx, saveTokeHashQuery, userId, tokenHash)
	return err
}

const tokenHashesQuery = `SELECT token_hash FROM refresh_tokens WHERE user_guid = $1 AND device_id = $2`
...
const replaceTokenHashQuery = `UPDATE refresh_tokens SET device_id = $3, token_hash = $4
...
func (r *refreshTokensRepository) UpdateTokenHash(
	...
	cmd, err := uow.Tx().Exec(ctx, replaceTokenHashQuery, userId, oldDeviceId, newDeviceId, tokenHash)
	if err != nil {
		return err
	}
